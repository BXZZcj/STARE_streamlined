...
# Adopted and modified from some open source code
...
def read_h5_event_components(hdf_path):
    ...
    with h5py.File(hdf_path, 'r') as f:
        if 'events/x' in f:
            ...
            t,x,y,p=np.split(f['events'],4,axis=1)
            t=np.squeeze(t).astype(int)
            x=np.squeeze(x).astype(int)
            y=np.squeeze(y).astype(int)
            p=np.squeeze(p).astype(int)
            t=t/1e6
            assert(len(t)==len(x) and len(t)==len(y) and len(t)==len(p))
            #print('{} events read in successfully'.format(len(t)))
...
def voxel_grids_fixed_n_torch(xs, ys, ts, ps, B, n, step=None, sensor_size=(180, 240),resize_factor=None, temporal_bilinear=True):
    ...
    if not step:
        step=n
    for idx in range(0, len(xs)-n+1, step):
        voxels.append(events_to_voxel_torch(xs[idx:idx+n], ys[idx:idx+n],
            ts[idx:idx+n], ps[idx:idx+n], B, sensor_size=sensor_size, resize_factor=resize_factor,
            temporal_bilinear=temporal_bilinear))
        #if idx>6*step:
        #    break
    return voxels

def voxel_grids_fixed_t_torch(xs, ys, ts, ps, B, t, sensor_size=(180, 240), resize_factor=1, temporal_bilinear=True):
    """
    ...
    for t_start in np.arange(ts[0].item(), ts[-1].item()-t, t):
        voxels.append(events_to_voxel_timesync_torch(xs, ys, ts, ps, B, t_start, t_start+t, np_ts=np_ts,
            sensor_size=sensor_size,resize_factor=resize_factor, temporal_bilinear=temporal_bilinear))
    return voxels

def events_to_voxel_timesync_torch(xs, ys, ts, ps, B, t0, t1, device=None, np_ts=None, sensor_size=(180, 240), resize_factor=None, temporal_bilinear=True):
    """
    ...
    if device is None:
        device = xs.device
    #TODO Optimize binary search, first consider avoiding full segment input
...
def events_to_voxel_torch(xs, ys, ts, ps, B, device=None, sensor_size=(180, 240),resize_factor=None, temporal_bilinear=True):
    ...
    bins = []
    if len(ts)==0:
         return torch.zeros((B,sensor_size[0],sensor_size[1]))
    dt = ts[-1]-ts[0]
    ...
    for bi in range(B):
        if temporal_bilinear:
            ...
            #print('sum of weights each bin:',bi,torch.sum(weights))
    ...
    if resize_factor is not None:
        ...
        transform=transforms.Resize((sensor_size[0]//resize_factor,sensor_size[1]//resize_factor), antialias=True)
        bins=transform(bins)
    return bins

def events_to_image_torch(xs, ys, ps, device=None, sensor_size=(180, 240) ,interpolation=None, padding=True):
    ...
    Method to turn event tensor to image. Allows for bilinear interpolation.
        ...
        :param interpolation: which interpolation(spatial) to use. Options=None,'bilinear'
    ...
    img = torch.zeros(img_size,dtype=torch.float64).to(device)
...
def events_to_neg_pos_voxel_torch(xs, ys, ts, ps, B, device=None,
    ...
    pos_weights = torch.where(ps>0, ones_v, zero_v).to(device)
    neg_weights = torch.where(ps<=0, ones_v, zero_v).to(device)
...
def events_to_neg_pos_voxel_fixed_t_torch(xs, ys, ts, ps, B, t,device=None,
        sensor_size=(180, 240),resize_factor=None, temporal_bilinear=True):
    '''
    New definition: Fixed time interval and positive and negative event separated voxel grids
    Return []voxels
    '''
    device = xs.device
    voxels = []
    ...
    length_ts=len(np_ts)
    search_field=int(length_ts//((np_ts[-1]-np_ts[0])/t)*2)
    
    for t_start in np.arange(ts[0].item(), ts[-1].item()-t, t):
        start_idx = np.searchsorted(np_ts, t_start)
        if (start_idx+search_field)>length_ts:
            end_idx=np.searchsorted(np_ts,t_start+t)
        else:
            end_idx = np.searchsorted(np_ts[:start_idx+search_field], t_start+t)
        #if >t[-1]:
        #xs_clip=xs[idx_start:idx_end]
        #ys_clip=ys[idx_start:idx_end]
        #ts_clip=ts[idx_start:idx_end]
        #pos_weights_clip=pos_weights[idx_start:idx_end]
        #neg_weights_clip=neg_weights[idx_start:idx_end]
        voxel_pos = events_to_voxel_torch(xs[start_idx:end_idx], ys[start_idx:end_idx],
        ts[start_idx:end_idx], pos_weights[start_idx:end_idx], B, device, sensor_size=sensor_size,resize_factor=resize_factor,
        temporal_bilinear=temporal_bilinear)
        voxel_neg = events_to_voxel_torch(xs[start_idx:end_idx], ys[start_idx:end_idx],
        ts[start_idx:end_idx], neg_weights[start_idx:end_idx], B, device, sensor_size=sensor_size,resize_factor=resize_factor,
        temporal_bilinear=temporal_bilinear)
...
def events_to_neg_pos_voxel_fixed_n_torch(xs, ys, ts, ps, B, n, step=None,device=None,
        sensor_size=(180, 240),resize_factor=None, temporal_bilinear=True):
    '''
        Given set of events, generate voxel grids
        return Tensor of shape [len,bins*2,H,W]
        where len is the number of volumes
    ...
    voxel_pos=voxel_grids_fixed_n_torch(xs=xs,ys=ys,ts=ts,ps=pos_weights,B=B,n=n,step=step,resize_factor=resize_factor,sensor_size=sensor_size)
    voxel_neg=voxel_grids_fixed_n_torch(xs=xs,ys=ys,ts=ts,ps=neg_weights,B=B,n=n,step=step,resize_factor=resize_factor,sensor_size=sensor_size)

    voxel=torch.cat((torch.stack(voxel_pos,dim=0),torch.stack(voxel_neg,dim=0)),dim=1)
    return voxel
...
def unpack_batched_events(events, batch_indices):
    ...
    for b_idx in range(len(batch_indices)):
        end_idx = batch_indices[b_idx]
        maxlen = end_idx-start_idx if end_idx-start_idx > maxlen else maxlen
        start_idx = end_idx
...
def events_to_voxel(xs, ys, ts, ps, B, sensor_size=(180, 240), temporal_bilinear=True):
    ...
    for bi in range(B):
        if temporal_bilinear:
            t_allo = t_norm-bi
            bilinear_weights = np.maximum(0, 1.0-np.abs(t_allo))
            weights = ps*bilinear_weights
        else:
...
def events_to_timestamp_image(xn, yn, ts, pn,
    ...
    if clip_out_of_range:
        clipx = img_size[1] if interpolation is None and padding==False else img_size[1]-1
        clipy = img_size[0] if interpolation is None and padding==False else img_size[0]-1
        mask = torch.where(xs>clipx, zero_v, ones_v)*torch.where(ys>clipy, zero_v, ones_v)  # modified from >= to >
    ...
    pos_weights = normalized_ts*pos_events_mask*mask
    neg_weights = normalized_ts*neg_events_mask*mask
    img_pos = torch.zeros(img_size).to(device)
    img_pos_cnt = torch.ones(img_size).to(device)
    img_neg = torch.zeros(img_size).to(device)
    img_neg_cnt = torch.ones(img_size).to(device)
    ...
    img = torch.zeros(img_size).to(device)
    ones = torch.ones(len(xs))
    img.index_put_((ys.long(),xs.long()), pos_weights, accumulate=True)
    
    return img_pos, img_neg,img #/img_pos_cnt, img_neg/img_neg_cnt
